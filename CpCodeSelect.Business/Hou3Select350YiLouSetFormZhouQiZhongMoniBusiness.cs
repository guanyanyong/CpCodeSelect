...
namespace CpCodeSelect.Business
{
    ...
    /// 模拟执行3个3轮的大小单双单双
    /// </summary>
    public class Hou3Select350YiLouSetFormZhouQiZhongMoniBusiness
    {
        ...
        private List<string> current350List=new List<string>();
        public Hou3Select350YiLouSetFormZhouQiZhongMoniBusiness(LogDelegate logMethod, List<Hou3Select350_ZhouQiZhong> model350List)
        {
        ...
        public void CalcCode(Code code)
        {
            ...
            else
            {
                ...
                {
                    //当前执行中 当前是中
                    if (current350List.Contains(code.GetHou3String()))
                    {
                        //执行中,中出
                        ...
                        LogInfo($"[{DateTime.Now:HH:mm:ss.fff}]-期号:{code.CodeQiHao},号码：{code.CodeNumber}，第{CurrentLun}轮第{CurrentaQi}期的【已中出,总额【{TotalResult}】,机会寻找中...");
