...
namespace CpCodeSelect.Business
{
    ...
    /// 模拟执行3个3轮的大小单双单双
    /// </summary>
    public class Five1MaBusinessMoniBusiness
    {
        public delegate void LogDelegate(string message);
        private LogDelegate _logMethod;
        public Five1MaBusinessMoniBusiness(LogDelegate logMethod)
        {
        ...
        /// 当前推荐号码
        /// </summary>
        public int CurrentTuiJianNumber { get; set; }
        ...
        public void CalcCode(Code code)
        {
            ...
            if (IsOriginBeginStatus())
            {
                ...
                var tuiJian = code.Five1MaModelList.Where(p => p.GuaCount == 0).FirstOrDefault();
                if (tuiJian != null)
                {
                    //找到新的推荐号码
                    CurrentTuiJianNumber = tuiJian.Number;
                    CurrentLun = 1;
                    CurrentaQi = 1;
                    CurrentBei = LunAmountMatrix[CurrentLun - 1, 0];
                    TotalResult = TotalResult - CurrentBei / 2;
                    TotalLiuShui += CurrentBei / 2;
                    LogInfo($"[{DateTime.Now:HH:mm:ss.fff}]-期号:{code.CodeQiHao},号码：{code.CodeNumber}，第{CurrentLun}轮第{CurrentLunZhongJiangCiShu + 1}次第{CurrentaQi}期,当前推荐【{CurrentTuiJianNumber}】,倍数是【{CurrentBei}】,总额【{TotalResult}】");
