            ...
            if (IsOriginBeginStatus())
            {
                ...
                if (positionNumber != null)
                {
                    ...
                    if (positionNumber.DanShuangLianKaiGuaCount == LunGuaTime[0])
                    {
                        ...
                        LogInfo($"[{DateTime.Now:HH:mm:ss.fff}]-期号:{code.CodeQiHao},号码：{code.CodeNumber}，第{CurrentLun}轮第{CurrentLunZhongJiangCiShu + 1}次第{CurrentaQi}期的{CurrentPositionType.ToString()}位的单双,倍数是{CurrentBei}");
                    }
                    else
                    {
                        //如果是大小连开挂
                        ...
                        LogInfo($"[{DateTime.Now:HH:mm:ss.fff}]-期号:{code.CodeQiHao},号码：{code.CodeNumber}，第{CurrentLun}轮第{CurrentLunZhongJiangCiShu + 1}次第{CurrentaQi}期的{CurrentPositionType.ToString()}位的大小,倍数是{CurrentBei}");
                    }
                }
            }
                ...
                if (position != null)
                {
                    if (IsDaXiao)
                    {
                        ...
                        if (lianGuaCount > LunGuaTime[CurrentLun - 1])
                        {
                            ...
                            if (CurrentaQi > 3)
                            {
                                ...
                                if (CurrentLun > TotalLun)
                                {
                                    ...
                                    LogInfo($"[{DateTime.Now:HH:mm:ss.fff}]-期号:{code.CodeQiHao},号码：{code.CodeNumber}，第{CurrentLun}轮第{CurrentLunZhongJiangCiShu + 1}次第{CurrentaQi - 1}期的{CurrentPositionType.ToString()}位的大小已挂,当前9期计划已挂********************");
                                    LunInit();
                                    return;
                                }
                                else
                            ...
                            else
                            {
                                ...
                                LogInfo($"[{DateTime.Now:HH:mm:ss.fff}]-期号:{code.CodeQiHao},号码：{code.CodeNumber}，第{CurrentLun}轮第{CurrentLunZhongJiangCiShu + 1}次第{CurrentaQi - 1}期的{CurrentPositionType.ToString()}位的大小已挂,进入下一期倍数是{CurrentBei}+++++");
                            }
                        }
                        else if (lianGuaCount == 0)
                        {
                            ...
                            {
                                //第一轮中出,则直接重新初始化
                                LogInfo($"[{DateTime.Now:HH:mm:ss.fff}]-期号:{code.CodeQiHao},号码：{code.CodeNumber}，第{CurrentLun}轮第{CurrentLunZhongJiangCiShu + 1}次第{CurrentaQi}期的{CurrentPositionType.ToString()}位的大小已中出,等待下次机会");
                                LunInit();
                            ...
                            else
                            {
                                ...
                                {
                                    LogInfo($"[{DateTime.Now:HH:mm:ss.fff}]-期号:{code.CodeQiHao},号码：{code.CodeNumber}，第{CurrentLun}轮第{CurrentLunZhongJiangCiShu}次第{CurrentaQi}期的{CurrentPositionType.ToString()}位的大小已中出,已中出2次,等待下次机会");
                                    //中出2轮,则重新初始化
                                    LunInit();
                                }
                                else
                                {
                                    //只中一轮,则进入下一轮
                                    LogInfo($"[{DateTime.Now:HH:mm:ss.fff}]-期号:{code.CodeQiHao},号码：{code.CodeNumber}，第{CurrentLun}轮第{CurrentLunZhongJiangCiShu}次第{CurrentaQi}期的{CurrentPositionType.ToString()}位的大小已中出,目前中出1次,本轮还需要再中一次,等待下次机会");
                                    CurrentaQi = 0;
                                    CurrentBei = 0;
                                }
                    ...
                    else
                    {
                        ...
                        if (lianGuaCount > LunGuaTime[CurrentLun - 1])
                        {
                            ...
                            if (CurrentaQi > 3)
                            {
                                ...
                                if (CurrentLun > TotalLun)
                                {
                                    ...
                                    //Todo 添加结束逻辑
                                    //LogInfo($"[{DateTime.Now:HH:mm:ss.fff}]-模拟结束,总金额是{TotalResult}");
                                    LogInfo($"[{DateTime.Now:HH:mm:ss.fff}]-期号:{code.CodeQiHao},号码：{code.CodeNumber}，第{CurrentLun}轮第{CurrentLunZhongJiangCiShu + 1}次第{CurrentaQi - 1}期的{CurrentPositionType.ToString()}位的单双已挂,当前9期计划已挂********************");
                                    LunInit();
                                    return;
                                }
                                else
                            ...
                            else
                            {
                                ...
                                LogInfo($"[{DateTime.Now:HH:mm:ss.fff}]-期号:{code.CodeQiHao},号码：{code.CodeNumber}，第{CurrentLun}轮第{CurrentLunZhongJiangCiShu + 1}次第{CurrentaQi - 1}期的{CurrentPositionType.ToString()}位的单双已挂,进入下一期倍数是{CurrentBei}+++++");
                        ...
                        else if (lianGuaCount == 0)
                        {
                            ...
                            {
                                //第一轮中出,则直接重新初始化
                                LogInfo($"[{DateTime.Now:HH:mm:ss.fff}]-期号:{code.CodeQiHao},号码：{code.CodeNumber}，第{CurrentLun}轮第{CurrentLunZhongJiangCiShu + 1}次第{CurrentaQi}期的{CurrentPositionType.ToString()}位的单双已中出,等待下次机会");
                                LunInit();
                                //LogInfo($"[{DateTime.Now:HH:mm:ss.fff}]-第一轮中出等待下一次");
                            ...
                            else
                            {
                                ...
                                if (CurrentLunZhongJiangCiShu == 2)
                                {
                                    LogInfo($"[{DateTime.Now:HH:mm:ss.fff}]-期号:{code.CodeQiHao},号码：{code.CodeNumber}，第{CurrentLun}轮第{CurrentLunZhongJiangCiShu}次第{CurrentaQi}期的{CurrentPositionType.ToString()}位的单双已中出,已中出2次,等待下次机会");
                                    //中出2轮,则重新初始化
                                    LunInit();
                                }
                                else
                                {
                                    //只中一轮,则进入下一轮
                                    LogInfo($"[{DateTime.Now:HH:mm:ss.fff}]-期号:{code.CodeQiHao},号码：{code.CodeNumber}，第{CurrentLun}轮第{CurrentLunZhongJiangCiShu}次第{CurrentaQi}期的{CurrentPositionType.ToString()}位的单双已中出,目前中出1次,本轮还需要再中一次,等待下次机会");
                                    CurrentaQi = 0;
                                    CurrentBei = 0;
                                }
