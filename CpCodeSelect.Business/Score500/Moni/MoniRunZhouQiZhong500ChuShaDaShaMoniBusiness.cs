...
namespace CpCodeSelect.Business.Score500.Moni
{
    ...
    /// 模拟执行6个6轮的确认点买入
    /// </summary>
    public class MoniRunZhouQiZhong500ChuShaDaShaMoniBusiness
    {
        ...
        public MoniRunZhouQiZhong500ChuShaDaShaMoniBusiness(LogDelegate logMethod, List<Hou3Select500_ZhouQiZhongScore> model350List)
        {
        ...
        private decimal[,] ZhongJiangAmountMatrix =
                ...
                {2.91M, 8.73M,20.37M, 44.62M,},
                {95.06M,198.85M,413.22M,855.54M,},
                {1768.31M,3652.05M,7539.81M,15563.62M},
        ...
        */
        ...
        public void CalcCode(Code code)
        {
            ...
            else
            {
                ...
                {
                    //当前是等待下一轮状态,说明上一轮已经结束,等待出啥跟啥中奖后开启下一轮
                    var model = currentSelect;
                    if (!(
                    model.PositionType == PositionType.万 && current350List.Contains(code.Wan.Number.ToString())
                    || model.PositionType == PositionType.千 && current350List.Contains(code.Qian.Number.ToString())
                    || model.PositionType == PositionType.百 && current350List.Contains(code.Bai.Number.ToString())
                    || model.PositionType == PositionType.十 && current350List.Contains(code.Shi.Number.ToString())
                    || model.PositionType == PositionType.个 && current350List.Contains(code.Ge.Number.ToString())
                    ))

                    {
                        //等待下一轮,没有中出,继续等待
                        LogInfo($"[{DateTime.Now:HH:mm:ss.fff}]-期号:{code.CodeQiHao},号码：{code.CodeNumber}，-挂了{CurrentLun - 1}轮2期 位置是:{currentSelect.PositionType},号码是{string.Join(" ", currentSelect.Number500)}，开奖号是:{code.CodeNumber} 等待中奖后进入下一轮");
                        CurrentBei = 0;
                        //等待的时候也要计算当前投注内容
                        GetCurrentTouZhuList(model, code);
                        return;
                    }
                    else
                    {
                        ...
                        LogInfo($"[{DateTime.Now:HH:mm:ss.fff}]-期号:{code.CodeQiHao},号码：{code.CodeNumber},位置是:{currentSelect.PositionType},号码是{string.Join(" ", current350List)}，-出啥跟啥中奖了 开启下一轮");
                        //如果当前中了,则是开启开一轮
                        //return;
                    }
                }
                ...
                {
                    var model = currentSelect;
                    //根据出啥打啥 判断是否中奖使用当前选择的号码,而不再是本轮的号码
                    //当前执行中 判断是否中奖不再是使用当前轮选中的号码,而是当前期的号码,因为是根据
...

                    {
                        ...
                        LogInfo($"[{DateTime.Now:HH:mm:ss.fff}]-期号:{code.CodeQiHao},号码：{code.CodeNumber},位置是:{currentSelect.PositionType},号码是{string.Join(" ", current350List)}，，第{CurrentLun}轮第{GuaCount}期已中出,中奖金额:{zhongjiangAmount}，总额【{TotalResult}】。");
        ...
        public void Select350AndStartCalc(Code code)
        {
            ...
            if (getEnoughRecord.Number500.Count > 0)
            {
                ...
                GetCurrentTouZhuList(currentSelect, code);
                //初始状态
                //第一期投注
                CurrentLun = 1;
                CurrentaQi = 1;
                ...
                LogInfo($"[{DateTime.Now:HH:mm:ss.fff}]-期号:{code.CodeQiHao},号码：{code.CodeNumber}，第{CurrentLun}轮第{CurrentaQi}期,位置是:{currentSelect.PositionType}，投注号码是:{string.Join(",", current350List)},下注金额【{CurrentAmount}】,投注后总额【{TotalResult}】");
                CurrentaQi = 2;
            }
        }
        /// <summary>
        ...
        private void GetCurrentTouZhuList(Hou3Select500_ZhouQiZhongScore model,Code code)
        {
            //这里判断的是 本轮的号码和当前期号码是否一样,一样的话就打本轮的号码,不一样的话就打相反的号码 本质上就是出啥打啥
            if (
            ...
            {
                // 如果当前中奖了,下一期选择同样的号码
                current350List = model.Number500;
            }
            else
            {
                // 如果当前期未中奖,下一期选择相反的5个号码
                current350List =GetRemainingDigits(model.Number500);
            }
        }
        /// <summary>
        /// 开始下一轮 从第一期开始
