...
namespace CpCodeSelect.Business.Score500.Moni
{
    ...
    /// 模拟执行6个6轮的确认点买入
    /// </summary>
    public class Hou3Select500ZiRanGenerate3JiaoMoniBusiness
    {
        public delegate void LogDelegate(string message);
        private LogDelegate _logMethod;
        private List<Hou3Select500_ZhouQiZhongScore> model500List = new List<Hou3Select500_ZhouQiZhongScore>();
        ...
        public List<string> FanXiangNumberList = new List<string>();
        public Hou3Select500ZiRanGenerate3JiaoMoniBusiness(LogDelegate logMethod, List<Hou3Select500_ZhouQiZhongScore> model500List)
        {
            _logMethod = logMethod ?? throw new ArgumentNullException(nameof(logMethod));
            this.model500List = model500List;
        ...
        */
        ...
        public List<string> ChuHao = new List<string>()
        {
            "反","正","反",
            "反","正","反",
            "反","正","反",
        ...
        public void CalcCode(Code code)
        {
            ...
            if (this.model500List == null || model500List.Count == 0)
            {
                model500List = Hou3Select500ZiRanGenerateBusiness.model500List;
            }
            ...
            else
            {
                ...
                //    if (!currentSelect.Number500.Contains(code.GetHou3String()))
                //    {
                //        //等待下一轮,没有中出,继续等待
                ...
                if (IsRunning)
                {
                    ...
                    int currentQiNumber = (CurrentLun - 1) * 3 + GuaCount;
                    bool isZhong = false;
                    if ((ChuHao[currentQiNumber - 1] == "正" && currentSelect.Number500.Contains(housanStr))
                        ||
                        (ChuHao[currentQiNumber - 1] == "反" && FanXiangNumberList.Contains(housanStr))
                        )
                        isZhong = true;
                    if (isZhong)
                    {
        ...
        public void Select350AndStartCalc(Code code)
        {
            ...
            //list = Hou3Select350YiLouSetFormZhouQiZhongBusiness.model500List.
                ...
                */
                ...
                foreach (var record in Hou3Select500ZiRanGenerateBusiness.model500List)
                {
                    ...
                    if (kLine500.Count >= 6)
                    {
                        ...
                        var kLin6 = boll[boll.Count - 6];

                        if( kLin1.YiLouGuaCount==0 && kLin2.YiLouGuaCount == 1 
                            && kLin3.YiLouGuaCount == 0 && kLin4.YiLouGuaCount == 1
                            && kLin5.YiLouGuaCount == 0 && kLin6.YiLouGuaCount == 1
                            )
                        {
                            getEnoughRecordList.Add(record);
                        }
                        ...
                        if (gap1 > 0.7 && gap2 > 0.7 && gap3 > 0.7)
                        {
                            //不要有轨超压 并且中的个数小于等于6
                            //if (kLin1.Bolling.BollUpperValue < kLin1.KValue && kLin1.CurrentZhongCount<=5 )
                            if (kLin1.Bolling.BollUpperValue < kLin1.KValue)
                            {
                                //轨距是增加
            ...
            if (getEnoughRecord.Number500.Count > 0)
            {
                ...
                FanXiangNumberList = GetRemainingDigits(getEnoughRecord.Number500);
                //选中后删除
                Hou3Select500ZiRanGenerateBusiness.model500List.Remove(currentSelect);
                //初始状态
                //第一期投注
                CurrentLun = 1;
                CurrentaQi = 1;
