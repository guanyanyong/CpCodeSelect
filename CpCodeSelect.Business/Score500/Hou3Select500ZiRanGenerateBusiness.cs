...
{
    public class Hou3Select500ZiRanGenerateBusiness : Hou3Select350YiLouSetFormZhongParentBusiness
    {
        ...
        /// 评分对象列表
        /// </summary>
        public static List<Hou3Select500_ZhouQiZhongScore> model500List = new List<Hou3Select500_ZhouQiZhongScore>();
        ...
        public static new void InitData()
        {
            ...
            model500List = new List<Hou3Select500_ZhouQiZhongScore>();
        ...
        public static void InitCode(Code code, bool zhongHouDelete = false)
        {
            ...
            Hou3Select500ZiRanGenerateBusiness.code = code;
        }
        /// <summary>
        ///  删除小于指定挂数的旧记录
        /// </summary>
        ...
        public static new void RemoveOldModel(Code code, int guaCount = 1)
        {
            ...
            while (model500List.Count >= numberCount)
            {
                for (int i = 0; i < 10; i++)
                {
                    var model = model500List[i];
            ...
            //超过30期就删除
            {
                ...
                for (int i = 0; i < model500List.Count; i++)
                {
                    var model = model500List[i];
                    var currentDecimal = Convert.ToDecimal(model.CodeQiHao);
        ...
        {
            foreach (var model in model500List.ToList())
            {
                CalcCode(code, model);
            }
        ...
        private static void CalcCode(Code code, Hou3Select500_ZhouQiZhongScore model)
        {
            ...
            if (model.Number500.Contains(housanStr))
            {
                //中了
                model.ZhongGount++;
        ...
        public static new void Generate500Code(Code code)
        {
            if (AllCode != null && AllCode.Count > RunSkipNumber)
            {
                ...
                */
                ...
                if (model500List.Count >= numberCount)
                    return;
                ...
                //每次生成15个
                var numerList = MultiThreadedNumberSelectFor500Hou3ZiRanGenerate.GenerateMultipleGroups(hou3List, 50);
                lock (Lockmodel500List)
                {
