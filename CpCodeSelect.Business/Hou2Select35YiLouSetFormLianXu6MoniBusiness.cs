...
namespace CpCodeSelect.Business
{
    ...
    /// 模拟执行1个6
    /// </summary>
    public class Hou2Select35YiLouSetFormLianXu6MoniBusiness
    {
        public delegate void LogDelegate(string message);
        private LogDelegate _logMethod;
        private List<Hou2Select35Model> model35List = new List<Hou2Select35Model>();
        public List<string> current35List = new List<string>();
        public List<string> before35List = new List<string>();
        public List<YilouStatistic> yilouStatisticList = new List<YilouStatistic>();
        private static readonly ThreadLocal<Random> _threadLocalRandom =
        new ThreadLocal<Random>(() => new Random(Guid.NewGuid().GetHashCode()));
        public Hou2Select35YiLouSetFormLianXu6MoniBusiness(LogDelegate logMethod, List<Hou2Select35Model> model35List)
        {
            _logMethod = logMethod ?? throw new ArgumentNullException(nameof(logMethod));
            this.model35List = model35List;
        ...
        public void CalcCode(Code code)
        {
            ...
            if (this.model35List == null || model35List.Count == 0)
            {
                model35List = Hou2Select35YiLouSetBusiness.model35List;
            }
            ...
            else
            {
                ...
                {
                    //当前执行中 当前是中
                    var housanStr = code.GetHou2String();
                    if (current35List.Contains(housanStr))
                    {
                        //执行中,中出
                        TotalZhong++;
                    ...
                    else
                    {
                        ...
                        else if (GuaCount == 7)
                        {
                            ...
                            //超过总轮次，结束
                            TotalGua++;
                            LogInfo($"[{DateTime.Now:HH:mm:ss.fff}]-期号:{code.CodeQiHao},号码：{code.CodeNumber}，已超过总轮次轮，结束本次执行。号码为:{string.Join(" ", current35List)}");
                            LogInfo($"[{DateTime.Now:HH:mm:ss.fff}]-总中奖次数{TotalZhong}，总额【{TotalResult}】。总挂次数{TotalGua}");
                            LunInit();
                            before35List = current35List;
                            Select35AndStartCalc(code);
                            ...
                            yilouStatisticList[7].TotalCount = yilouStatisticList[7].TotalCount + 1;
                        }
                    }
                }
                else
        ...
        /// 查找满足条件的号码并开始执行
        /// </summary>
        public void Select35AndStartCalc(Code code)
        {
            //list = Hou3Select350YiLouSetFormZhouQiZhongBusiness.model350List.
            var list = model35List.Where(p => p.GuaCount == 1).ToList();
            Hou2Select35Model record = null;
            ...
            if (record != null && record.Number35.Count > 0)
            {
                IsRunning = true;
                current35List = record.Number35;
                //初始状态
                //第一期投注
                CurrentLun = 1;
                CurrentaQi = 1;
