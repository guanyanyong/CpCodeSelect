...
namespace CpCodeSelect.Business
{
    ...
    /// 模拟执行4个2轮的确认点买入
    /// </summary>
    public class Hou3Select270YiLouSetFormZhouQiZhongMoniBusiness
    {
        ...
        public Hou3Select270YiLouSetFormZhouQiZhongMoniBusiness(LogDelegate logMethod, List<Hou3Select270_ZhouQiZhong> model350List)
        {
            _logMethod = logMethod ?? throw new ArgumentNullException(nameof(logMethod));
            this.model270List = model350List;
        ...
        public void Select350AndStartCalc(Code code)
        {
            ...
            {
                IsRunning = true;
                current350List = record.Number270;
                //初始状态
                //第一期投注
                CurrentLun = 1;
                CurrentaQi = 1;
