...
namespace CpCodeSelect.Business
{
    ...
    /// 模拟执行4个2轮的确认点买入
    /// </summary>
    public class Hou3Select350YiLouSetFormZhouQiZhongLianXu8Amount5566MoniBusiness
    {
        private bool UserFirstRecord = true;
        ...
        public Hou3Select350YiLouSetFormZhouQiZhongLianXu8Amount5566MoniBusiness(LogDelegate logMethod, List<Hou3Select350_ZhouQiZhong> model350List)
        {
            _logMethod = logMethod ?? throw new ArgumentNullException(nameof(logMethod));
            this.model350List = model350List;
            var useFirstRecord = ConfigurationManager.AppSettings["5566UserFirstRecord"];
            if (!string.IsNullOrEmpty(useFirstRecord))
            {
                UserFirstRecord = useFirstRecord == "1";
            }
            else
            {
                UserFirstRecord = true;
            }
        ...
        private decimal[,] LunAmountMatrix = {
                    { 56.7M, 88.55M,138.6M, 217M,339.5M, 339.5M,530.95M, 530.95M},
                };
        private decimal[,] ZhongJiangAmountMatrix = {
                    { 157.14M, 245.41M ,384.12M, 601.4M,940.9M, 940.9M ,1471.49M, 1471.49M}
                };
        /// <summary>
        /// 总金额
        /// </summary>
        ...
        public void Select350AndStartCalc(Code code)
        {
            ...
            else if (!UserFirstRecord && list.Count >= 2)
            {
                record = list[1];
            }
