...
namespace CpCodeSelect.Business
{
    ...
    /// 模拟执行4个2轮的确认点买入
    /// </summary>
    public class Hou3Select350YiLouSetFormDuoZhouQiZhong8ge1YiLou1MoniBusiness
    {
        ...
        public Hou3Select350YiLouSetFormDuoZhouQiZhong8ge1YiLou1MoniBusiness(LogDelegate logMethod, List<Hou3Select350_ZhouQiZhong> model350List)
        {
            ...
            else
            {
                ...
                if (IsRunning)
                {
                    ...
                    if (current350List.Contains(housanStr))
                    {
                        ...
                        int zhongjiangqi = (CurrentLun - 1) * 1 + GuaCount;
                    ...
                    else
                    {
                        ...
                        {
                            LogInfo($"[{DateTime.Now:HH:mm:ss.fff}]-期号:{code.CodeQiHao},号码：{code.CodeNumber}，轮次{CurrentLun-1 }轮已结束，准备下一轮。350注号码是{string.Join(" ", current350List)}");
                        }
                        before350List = current350List;
