...
namespace CpCodeSelect.Util
{
    public class Generate270Code
    {
        ...
        {
            if (random == null) random = Generate270Code.random;
            ...
            // 2. 快速找到子集B（110-120个）
            var subsetB = FindSubsetB_Fast(setA, 80, 90, random);

            // 3. 生成集合C（A中没有的字符串）
            var setC = allPossibleStrings.Except(setA.Distinct()).ToList();

            // 4. 计算需要从C中取的数量
            var uniqueB = subsetB.Distinct().Count();
            var neededFromC = Math.Max(0, 270 - uniqueB);

            // 5. 从C中取字符串形成D
            List<string> setD = GenerateSetD_Random(setC, neededFromC, random);
            // 6. 合并B(去重)和D形成集合E (270个不重复字符串)
            List<string> setE = GenerateSetE(subsetB, setD);
            return setE;
        }
