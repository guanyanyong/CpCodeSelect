...
namespace CpCodeSelect.Util
{
    public class Generate350Code
    {
        ...
        {
            if (random == null) random = Generate350Code.random;
            ...
            var neededFromC = Math.Max(0, 350 - uniqueB);

            // 5. 从C中取字符串形成D
            List<string> setD = GenerateSetD_Random(setC, neededFromC, random);
            // 6. 合并B(去重)和D形成集合E (350个不重复字符串)
