...
{
    public class KLine40951ScoreCalc
    {
        ...
        public static Hou3Select156_ZhouQiZhong CalcKlineCurrent(Hou3Select156_ZhouQiZhong model, Code code)
        {
            ...
            if (model.KLineList.Count == 0)
            {
                ...
                //判断是否中奖
                if (model.Number156.Contains(code.CodeNumber))
                {
                    //中了 K值加1.857
                    kline.KValue = 1.441943;
            ...
            else
            {
                ...
                {
                    //中了 K值加1.857
                    kline.KValue = model.KLineList[model.KLineList.Count - 1].KValue + 1.441943;
            ...
            for (; runCount > 0; runCount--)
            {
                ...
                else
                {
                    ...
                    if (model.Number156.Contains(code.CodeNumber))
                    {
                        yiLouTuKLine = new KLine156();
                        //中了 K值加1.857
                        kline.KValue = beforeKLine.KValue + 1.441943;
        ...
        {
            var Scoring40951Engine = new Scoring40951Engine();
            ScorerUtil40951.InitializeScoringRulesForEngine(Scoring40951Engine);
            ...
            for (int i = historyData.Count - 2; i < historyData.Count; i++)
            {
                ...
                historyData[i].Score = Scoring40951Engine.CalculateTotalScore(
                    historyData[i],
                    historyForScoring
                );
                ...
                if (HistoryData.Count >= 1) // 至少有1期历史数据
                {
                    ...
                    if (lastBigGapIndex >= 0)
                    {
                        ...
                        if (firstWinAfterGap >= 0)
                        {
                            ...
                            // 当前期是否在理论周期内（遗漏值≤5 且 自从第一次中奖以来期数不超过6）
                            bool isInCycle = currentData.YiLouValue <= 0 && periodsSinceFirstWin <= 1;
                            ...
                            if (isInCycle)
                            {
                                ...
                                for (int i = firstWinAfterGap; i < HistoryData.Count; i++)
                                {
                                    if (HistoryData[i].IsZhongJiang)
                                    {
                                        // 检查该中奖期是否在理论周期内（遗漏值 <= 6）
                                        if (HistoryData[i].YiLouValue > 2)
                                        {
                                            allInCycle = false;
                                            break;
                                        }
        ...
        private static void CheckTrendSegment(LotteryScoreData currentData, Hou3Select156_ZhouQiZhong model)
        {
            ...
            if (latestBigGapIndex >= 0)
            {
                ...
                {
                    // 只统计中奖且在理论周期内（遗漏值≤5）的
                    if (HistoryData[i].IsZhongJiang && HistoryData[i].YiLouValue <= 5)
                    {
                        winsAfterBigGap++;
                    }
                }

                // 如果当前期也在理论周期内（遗漏值≤5）且中奖，则计入
        ...
        public static CheckResult KLineIsEnoughAddBuLinTopGui(List<KLine156> kLineList)
        {
            ...
            if (checkResult.Result)
            {
                ...
                //再加一个布林上轨的判断
                if (kLine.Bolling != null && (kLine.KValue + 2.7) >= kLine.Bolling.BollUpperValue)
                {
        ...
        public static CheckResult ADXIsEnough(List<KLine156> kLineList)
        {
        ...
        public static CheckResult MACDIsEnough(List<KLine156> kLineList)
        {
            var checkResult = new CheckResult();
            checkResult.Result = true;
            var count = kLineList.Count;
