...
namespace CpCodeSelect
{
    public partial class Zu6Kill1ZG2Form : Form
    {
        ...
        {
            Zu6Kill1ZG2Business.InitCode(code);
            AddToLogFileZu6Kill1(code, "Zu6Kill1ZG2.txt");
        ...
        private void AddToListBox(Code code)
        {
            ...
            if (code.Zu6Kill1ZG2ModelList != null && code.Zu6Kill1ZG2ModelList.Count > 0)
            {
                foreach (var zu6Kill1Mode in code.Zu6Kill1ZG2ModelList)
                {

                    var list = zu6Kill1Mode.Zu6Kill1ZG2Items.Where(p => p.IsZG2 && p.ZG2Count >= NeedGuaCount).ToList();
                    ...
                    if (list.Count > 0)
                    {
                        foreach (var record in list)
                        {
                            ...
                            {
                                //连挂中
                                AddStatisticToDic(record.ZG2Count, new StatisticModel()
                                {
                                    CodeNumber = code.CodeNumber,
                                    CodeQiHao = code.CodeQiHao,
                                    StatisticType = zu6Kill1Mode.Name,
                                    GuaCount = record.ZG2Count,
                                    Number = record.Number,
                                    GuaHouZhong = record.ZGGTZhongCount
                                });
                                if (chkLianGua.Checked)
                                {
                                    listBoxTuiJian.Items.Add($"[{DateTime.Now:HH:mm:ss.fff}] 记录 #" + $"期号:{code.CodeQiHao},号码：{code.CodeNumber}的{zu6Kill1Mode.Name}{record.Number}连挂中,连挂次数{record.ZG2Count},挂后挂{record.GuaCount}次。");
                                    needSlip = true;

                                    if (record.GuaCount <= 1)
                                    {
                                        needPlay = true;
                                    }
                                }
