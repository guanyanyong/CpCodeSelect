...
{
    public partial class Hou2Select50YiLouSetForm3guashangForm : Form
    {
        ...
        {
            Hou2Select50YiLouSetForm3guashangBusiness.InitCode(code);
            InitOfferNumber();
            GenerateOfferNumber();
            SetForm();
        ...
        {
            var needAddList = Hou2Select50YiLouSetForm3guashangBusiness.modelList.Where(p => p.NeedZhong == false && p.Gua3TimeCount >= boFangYanhuaCount).OrderByDescending(p => p.GuaCount).ToList();
        ...
        public void SetForm()
        {
            if (Hou2Select50YiLouSetForm3guashangBusiness.modelList.Where(p => p.NeedZhong == false).ToList().Count > 0)
            {

                lblMaxGua.Text = Hou2Select50YiLouSetForm3guashangBusiness.modelList.Where(p => p.NeedZhong == false).Max(p => p.GuaCount).ToString();
                lblTotalNumber.Text = Hou2Select50YiLouSetForm3guashangBusiness.modelList.Count.ToString();
        ...
        {
            bool needPlay = false;
            if (Hou2Select50YiLouSetForm3guashangBusiness.modelList.Count > 500)
            {
                ...
                {
                    var maxNumber = Hou2Select50YiLouSetForm3guashangBusiness.modelList.Where(p => p.NeedZhong == false).Max(p => p.Gua3TimeCount);
                    if (maxNumber >= boFangYanhuaCount)
                    {
                        needFlush = true;
                        Hou2Select50YiLouSetForm3guashangBusiness.modelList.Where(p => p.Gua3TimeCount == maxNumber).ToList().ForEach(recode =>
                        {
        ...
        private void dataGridView1_CellContentClick(object sender, DataGridViewCellEventArgs e)
        {
            ...
            if (e.RowIndex >= 0 && dataGridView1.Columns[e.ColumnIndex].Name == "Numer50")
            {
                ...
                if (row.DataBoundItem is Hou2Select50YiLouSetForm3guashangModel)
                {
                    var model = row.DataBoundItem as Hou2Select50YiLouSetForm3guashangModel;
        ...
        private void BtnSelectClick()
        {
            ...
            var list = Hou2Select50YiLouSetForm3guashangBusiness.modelList.Where(p => p.NeedZhong == false && p.Gua3TimeCount >= number).OrderByDescending(p => p.Gua3TimeCount).ThenByDescending(p=>p.GuaCount).ToList();
        ...
        public void GenerateCode(int number, List<Code> codeList)
        {
            ...
            {
                var codeStr = Hou2Select50YiLouSetForm3guashangBusiness.GetHou2_50NumerString();
                Code code = new Code();
