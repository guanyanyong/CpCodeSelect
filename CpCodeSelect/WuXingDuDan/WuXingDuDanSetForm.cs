...
using CpCodeSelect.WuXingDuDan.Moni;

namespace CpCodeSelect.WuXingDuDan
{
    public partial class WuXingDuDanSetForm : Form
    {
        ...
        private MoniWuXingDuDan moniRunZhouQiZhongScore3ge5AfterZhong = new MoniWuXingDuDan();
        //private MoniRunZhouQiZhongScore8ge1 moniRunZhouQiZhongLianXu8 = new MoniRunZhouQiZhongScore8ge1();

        private MoniWuXingDuDan moniRunZhouQiZhongLianXu8 = new MoniWuXingDuDan();
        ...
        private MoniWuXingDuDan moniRunZhouQiZhong3Ge3 = new MoniWuXingDuDan();


        private MoniWuXingDuDan moni1 = new MoniWuXingDuDan();
        private MoniWuXingDuDan moni2 = new MoniWuXingDuDan();
        private MoniWuXingDuDan moni3 = new MoniWuXingDuDan();
        private MoniWuXingDuDan moni4 = new MoniWuXingDuDan();
        private MoniWuXingDuDan moni5 = new MoniWuXingDuDan();
        private MoniWuXingDuDan moni6 = new MoniWuXingDuDan();
        ...
        public void AnalySisCode(Code code, bool zhongHouDelete = false)
        {
            ...
            WuXingDuDanBusiness.InitCode(code, zhongHouDelete);
            InitOfferNumber();
            GenerateOfferNumber();
            SetForm();
        ...
        {
            var needAddList = WuXingDuDanBusiness.model350List.Where(p => p.NeedZhong == false && p.GuaCount >= boFangYanhuaCount).OrderByDescending(p => p.GuaCount).ToList();
            ...
            if (needAddList.Count > 0)
            {
                foreach (var model in needAddList)
                {
                    ...
                    exModel.五十码 = string.Join(" ", model.Number156);
        ...
        public void SetForm()
        {
            if (WuXingDuDanBusiness.model350List.ToList().Count > 0)
            {

                lblMaxGua.Text = WuXingDuDanBusiness.model350List.Max(p => p.GuaCount).ToString();
                lblTotalNumber.Text = WuXingDuDanBusiness.model350List.Count.ToString();
        ...
        {
            bool needPlay = false;
            if (WuXingDuDanBusiness.model350List.Count > 500)
            {
                ...
                {
                    var list = WuXingDuDanBusiness.model350List.Where(p => p.NeedZhong == false).ToList();
                    if (list.Count > 0)
                    {
                        ...
                        {
                            needFlush = true;
                            WuXingDuDanBusiness.model350List.Where(p => p.GuaCount == maxNumber).ToList().ForEach(recode =>
                            {
        ...
        private async void dataGridView1_CellContentClick(object sender, DataGridViewCellEventArgs e)
        {
            ...
            if (e.RowIndex >= 0 && dataGridView1.Columns[e.ColumnIndex].Name == "Numer50")
            {
                ...
                if (row.DataBoundItem is Hou3Select156_ZhouQiZhongScore)
                {
                    var model = row.DataBoundItem as Hou3Select156_ZhouQiZhongScore;
                    txt50Number.Text = string.Join(" ", model.Number156);
            ...
            else if (e.RowIndex >= 0 && dataGridView1.Columns[e.ColumnIndex].Name == "Delete")
            {
                ...
                {
                    var model = row.DataBoundItem as Hou3Select156_ZhouQiZhongScore;
                    WuXingDuDanBusiness.model350List.Remove(model);
        ...
        private void BtnSelectClick()
        {
            ...
            var list = WuXingDuDanBusiness.model350List.Where(p => p.NeedZhong == false && p.ZhouQiZhongHouGua >= number).OrderByDescending(p => p.ZhouQiZhongHouGua).ThenByDescending(p => p.IsZhouQiZhongHou).ToList();
            var number2 = numericUpDown2.Value;
            var guaCount = numericUpDown3.Value;
            if (number2 >= 0 && number <= number2)
            {
                list = WuXingDuDanBusiness.model350List.Where(p => p.NeedZhong == false && p.ZhouQiZhongHouGua >= number && p.ZhouQiZhongHouGua <= number2).OrderByDescending(p => p.ZhouQiZhongHouGua).ThenByDescending(p => p.IsZhouQiZhongHou).ToList();
            }
            if (guaCount > -1)
            {
        ...
        private void SetDataSource(List<Hou3Select156_ZhouQiZhongScore> list)
        {
            dataGridView1.DataSource = list;
            lblResultCount.Text = list.Count.ToString();
            dataGridView1.AutoResizeColumns(DataGridViewAutoSizeColumnsMode.AllCells);
        ...
        private void button2_Click_1(object sender, EventArgs e)
        {
            ...
            while (true)
            {
                ...
                var AllCode = WuXingDuDanBusiness.AllCode;
        ...
        {
            var hou3List = WuXingDuDanBusiness.GenerateHou3NumbereFromCode(270);
        ...
        private void btnTestCode_Click(object sender, EventArgs e)
        {
            ...
            Code code = WuXingDuDanBusiness.code;
            ...
            KLine156ScoreCalc.CalcKLineHistoryList(model350, WuXingDuDanBusiness.AllCode, 100);
        ...
        {
            var guaCount = numericUpDown3.Value;
            var list = WuXingDuDanBusiness.model350List.Where(p => p.NeedZhong == false && p.ZhouQiZhongHouGua == 0 && p.GuaCount == guaCount && p.IsZhouQiZhongHou).ToList();
            List<Hou3Select156_ZhouQiZhongScore> recordList = new List<Hou3Select156_ZhouQiZhongScore>();
        ...
        private void DataLoadKLine()
        {
            if (currentCalcKLineDate == null)
            {
                ...
                {
                    //自动加载最新数据
                    var modelList = WuXingDuDanBusiness.model350List;
                    if (modelList != null && modelList.Count > 0)
                    {
                        foreach (var record in modelList)
                        {
                            ...
                            WuXingDuDanBusiness.currentNeedCalcList.Add(currentCalcKLineDate);
                ...
                else
                {
                    ...
                    if (!string.IsNullOrEmpty(txtNumber))
                    {
                        ...
                        model350.Number156 = txtNumber.Split(' ').ToList();
                        model350.CodeNumber = WuXingDuDanBusiness.code.CodeNumber;
                        model350.CodeQiHao = WuXingDuDanBusiness.code.CodeQiHao;
                        model350.NeedZhong = true;
                        model350.KLineList = new List<KLine156>();
                        model350.YiLouKline350 = new List<YiLouKline350>();
                        model350.YiLouTuLineList = new List<KLine156>();

                        KLine156ScoreCalc.CalcKLineHistoryList(model350, WuXingDuDanBusiness.AllCode, (int)numericUpDown5.Value + 21);
        ...
        private void RemoveRecord(Hou3Select156_ZhouQiZhongScore record)
        {
            if (record != null)
            {
                WuXingDuDanBusiness.currentNeedCalcList.Remove(record);
            }

        }


        private void GenerateKLine(Hou3Select156_ZhouQiZhongScore data)
        {
            if (data.KLineList != null && data.KLineList.Count > 30)
            {
                ...
                textBox2.Text = string.Join(" ", data.Number156);
        ...
        private void GenerateYiLouKLine(Hou3Select156_ZhouQiZhongScore data)
        {
        ...
        private void btnYiLou_Click(object sender, EventArgs e)
        {
            ...
            if (WuXingDuDanBusiness.model350List.Count >= 350)
            {
                foreach (var record in WuXingDuDanBusiness.model350List)
                {
        ...
        private void btnScore_Click(object sender, EventArgs e)
        {
            ...
            /*
            if (WuXingDuDanBusiness.model350List.Count >= 25)
            {
                foreach (var record in WuXingDuDanBusiness.model350List)
                {
