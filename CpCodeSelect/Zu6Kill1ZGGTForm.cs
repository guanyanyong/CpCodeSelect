...
{
    public partial class Zu6Kill1ZGGTForm : Form
    {
        ...
        public void AnalySisCode(Code code)
        {
            ...
            AddToLogFileZu6Kill1(code, "Zu6Kill1ZGGT.txt");
            ...
            using (var writer = new StreamWriter(zu6Kill1FileName, true))
            {
                if (code.Zu6Kill1ZGGTModelList != null && code.Zu6Kill1ZGGTModelList.Count > 0)
                {
                    foreach (var zu6Kill1Mode in code.Zu6Kill1ZGGTModelList)
                    {
                        ...
                        var list = zu6Kill1Mode.Zu6Kill1ZGGTItems.Where(p => (p.IsZGGT && p.ZGGTGuaCount >= 1)).ToList();
                        if (list.Count > 0)
                        {
                            foreach (var recode in list)
                            {
                                needFlush = true;
                                //连挂中
                                writer.WriteLine($"[{DateTime.Now:HH:mm:ss.fff}] 记录 #" + $"期号:{code.CodeQiHao},号码：{code.CodeNumber}，{zu6Kill1Mode.Name}的{recode.Number}连挂中,当前ZGGT连挂次数为{recode.ZGGTGuaCount},挂后中{recode.ZGGTZhongCount}次");
                            }
                        }
                        if (needFlush)
                        {
        ...
        private void AddToListBox(Code code)
        {
            ...
            if (code.Zu6Kill1ZGGTModelList != null && code.Zu6Kill1ZGGTModelList.Count > 0)
            {
                foreach (var zu6Kill1Mode in code.Zu6Kill1ZGGTModelList)
                {
                    ...
                    var list = zu6Kill1Mode.Zu6Kill1ZGGTItems.Where(p => (p.IsZGGT && p.ZGGTGuaCount >= NeedGuaCount) || (!p.IsLianGua && p.LianZhongCount >= NeedGuaCount)).ToList();
                    if (!chkLianZhong.Checked)
                    {
                        list = list.Where(p => p.IsZGGT && p.ZGGTGuaCount >= NeedGuaCount).ToList();
                    }
                    if (list.Count > 0)
                    {
                        foreach (var record in list)
                        {
                            ...
                            {
                                //连挂中
                                AddStatisticToDic(record.ZGGTGuaCount, new StatisticModel()
                                {
                                    CodeNumber = code.CodeNumber,
                                    CodeQiHao = code.CodeQiHao,
                                    StatisticType = zu6Kill1Mode.Name,
                                    GuaCount = record.ZGGTGuaCount,
                                    Number = record.Number,
                                    GuaHouZhong = record.ZGGTZhongCount
                                });
                                if (chkLianGua.Checked)
                                {
                                    listBoxTuiJian.Items.Add($"[{DateTime.Now:HH:mm:ss.fff}] 记录 #" + $"期号:{code.CodeQiHao},号码：{code.CodeNumber}的{zu6Kill1Mode.Name}{record.Number}连挂中,连挂次数{record.ZGGTGuaCount},挂后中{record.ZGGTZhongCount}次。");
                                    needSlip = true;

                                    if (record.ZGGTZhongCount == 1)
                                    {
                                        needPlay = true;
                                    }
                                }
                            }
                            if (record.LianZhongCount >= NeedGuaCount && chkLianZhong.Checked)
                            {
                                // 连中
            ...
            if (needPlay)
            {
                ...
                using (SoundPlayer player = new SoundPlayer(".\\data\\hint.wav")) // 替换为你的音乐文件路径
                {
                    // 播放音乐
                    player.Play();
                }
        ...
        {
            Zu6Kill1ZGGTTxtFileExecForm form = new Zu6Kill1ZGGTTxtFileExecForm();
            form.ShowDialog();
        }
    }
}
